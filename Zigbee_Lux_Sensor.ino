...
#define HA_ESP_LIGHT_ENDPOINT       12                                   /* esp light bulb device endpoint, used to process light controlling commands */
#define ESP_ZB_PRIMARY_CHANNEL_MASK ESP_ZB_TRANSCEIVER_ALL_CHANNELS_MASK /* Zigbee primary channel mask use in the example */

/* Temperature sensor configuration */
#define ESP_TEMP_SENSOR_UPDATE_INTERVAL (1)  /* Local sensor update interval (second) */
#define ESP_TEMP_SENSOR_MIN_VALUE       (0) /* Local sensor min measured value (degree Celsius) */
#define ESP_TEMP_SENSOR_MAX_VALUE       (50) /* Local sensor max measured value (degree Celsius) */

/* Attribute values in ZCL string format
 * The string should be started with the length of its own.
 */
char modelid[] = {11, 'E', 'S', 'P', '3', '2', 'H', '2', '.', 'L', 'u', 'x'};
char manufname[] = {9, 'E', 's', 'p', 'r', 'e', 's', 's', 'i', 'f'};

Adafruit_TSL2591 tsl = Adafruit_TSL2591(2591); // pass in a number for the sensor identifier (for your use later)

/********************* Zigbee functions **************************/
static int16_t zb_temperature_to_s16(float MeasuredValue) {
  return (int16_t)(log10(MeasuredValue)*10000)+1;
}
...
static void esp_app_temp_sensor_handler(float temperature) {
  ...
  Serial.println("Updating temperature sensor value 2...");
  Serial.println(measured_value);
  /* Update temperature sensor measured value */
  esp_zb_lock_acquire(portMAX_DELAY);
  esp_zb_zcl_set_attribute_val(
    HA_ESP_SENSOR_ENDPOINT, ESP_ZB_ZCL_CLUSTER_ID_ILLUMINANCE_MEASUREMENT, ESP_ZB_ZCL_CLUSTER_SERVER_ROLE, ESP_ZB_ZCL_ATTR_ILLUMINANCE_MEASUREMENT_MEASURED_VALUE_ID, &measured_value,
    false
  );
  esp_zb_lock_release();
}
...
static esp_zb_cluster_list_t *custom_temperature_sensor_clusters_create(esp_zb_illuminance_meas_cluster_cfg_t *illuminance_sensor) 
{
  log_i("cp3");
...
static esp_zb_ep_list_t *custom_temperature_sensor_ep_create(uint8_t endpoint_id, esp_zb_illuminance_meas_cluster_cfg_t *illuminance_sensor)
{
  ...
  log_i("cp2");
  esp_zb_ep_list_add_ep(ep_list, custom_temperature_sensor_clusters_create(illuminance_sensor), endpoint_config);
  return ep_list;
}
...
static void esp_zb_task(void *pvParameters) {
  ...
  esp_zb_illuminance_meas_cluster_cfg_t sensor_cfg;
  ...
  /*esp_zb_zcl_reporting_info_t reporting_info = {
    ...
    .attr_id = ESP_ZB_ZCL_ATTR_ILLUMINANCE_MEASUREMENT_VALUE_ID,
    .u =
      {
        .send_info =
          {
...
static void temp_sensor_value_update(void *arg) {
  for (;;) {
    ...
    esp_app_temp_sensor_handler(lux);
    delay(1000);
  }
}
